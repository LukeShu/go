...
func (b *batch) flowClosure(k hole, clo *ir.ClosureExpr) {
	for _, cv := range clo.Func.ClosureVars {
		...
		logopt.LogOpt(n.Pos(), 1, "capture", "escape", fmt.Sprintf("%v capturing by %s: %v (addr=%v assign=%v width=%d)", n.Curfn, how, n, loc.addrtaken, loc.reassigned, n.Type().Size()))
	...
	for _, loc := range b.allLocs {
		...
		if loc.escapes {
			if n.Op() == ir.ONAME {
				...
				logopt.LogOpt(n.Pos(), 0, "move", "escape", fmt.Sprintf("moved to heap: %v", n))
			} else {
				logopt.LogOpt(n.Pos(), 0, "escape", "escape", fmt.Sprintf("%v escapes to heap", n))
			}
			n.SetEsc(ir.EscHeap)
		} else {
			if n.Op() != ir.ONAME {
				logopt.LogOpt(n.Pos(), 0, "noEscape", "escape", fmt.Sprintf("%v does not escape", n))
			}
			n.SetEsc(ir.EscNone)
			if loc.transient {
				switch n.Op() {
...
func (b *batch) paramTag(fn *ir.Func, narg int, f *types.Field) string {
	...
	if len(fn.Body) == 0 {
		...
		if f.Type.IsUintptr() {
			logopt.LogOpt(pos, 0, "uintptr", "escape", fmt.Sprintf("assuming %v is unsafe uintptr", name()))
			return ""
		}
		...
		if fn.Pragma&ir.Noescape != 0 {
			if f.Sym != nil {
				logopt.LogOpt(pos, 0, "noEscape", "escape", fmt.Sprintf("%v does not escape", name()))
			}
		} else {
			if f.Sym != nil {
				logopt.LogOpt(pos, 0, "leak", "escape", fmt.Sprintf("leaking param: %v", name()))
			}
			esc.AddHeap(0)
		}
