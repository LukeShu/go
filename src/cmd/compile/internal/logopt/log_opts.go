...
const (
	...
	fmtText            // human-readable plaintext
	fmtJson0           // version 0 for LSP 3.14, 3.15; future versions of LSP may change the format and the compiler may need to support both as clients are updated.
)

type logVerbosity uint8

var (
	flagVerbosity logVerbosity = -1
	flagFormat    logFormat    = fmtNone
	flagDest      string
)
...
func LogJsonOption(flagValue string) {
	...
	flagVerbosity = 4
	flagDest = checkLogPath(directory)
	flagFormat = fmtJson0
}

// LogTextOption handles a number attached to the -m compiler flag.
func LogTextOption(m int) {
	if m < -1 {
		m = -1
	}
	if m > 4 {
	...
	if logVerbosity(m) > flagVerbosity {
		flagVerbosity = logVerbosity(m)
	}
	if m > 0 {
		flagFormat = fmtText
	}
...
// Pos is the source position (including inlining), what is the message, pass is which pass created the message,
// funcName is the name of the function.
func LogOpt(pos src.XPos, lvl logVerbosity, what, pass, funcName string, args ...interface{}) {
	if flagVerbosity < lvl {
		return
	}
	switch flagFormat {
	case fmtNone:
		return
	case fmtText:
		if pos == base.AutogeneratedPos {
			return
		}
		base.WarnfAt(pos, "%s", fmt.Sprintln(args...))
	case fmtJson0:
...
// Enabled returns whether optimization logging is enabled at a given
// verbosity.g
func Enabled(lvl logVerbosity) bool {
	return flagVerbosity >= lvl
}
...
func FlushLoggedOpts(ctxt *obj.Link, slashPkgPath string) {
	...
	case fmtJson0: // LSP 3.15
		...
		subdirpath := filepath.Join(flagDest, url.PathEscape(slashPkgPath))
